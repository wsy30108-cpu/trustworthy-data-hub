...
import { useMLModelStore, type MLModel, type MLModelVersion } from "@/stores/useMLModelStore";

function versionSourceLabel(source?: MLModelVersion["source"]) {
  return (source ?? "manual") === "active_learning" ? "主动学习" : "手动添加";
}

function copyProcessedDisplay(m: MLModel, v: MLModelVersion) {
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            <p className="text-xs text-muted-foreground">模型副本管理</p>
          </div>
        </div>
      </div>
        ...
        <>
          ...
          <div className="flex justify-end">
            <Button size="sm" onClick={openCreate} className="h-8 gap-1.5 text-xs">
              <Plus className="w-3.5 h-3.5" /> 新增副本
            </Button>
          </div>
          ...
          <div className="rounded-lg border bg-card overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-muted/30 border-b">
                    <th className="text-left py-3 px-3 text-xs font-medium text-muted-foreground">副本名称</th>
                  ...
                  {currentVersions.map((v) => {
                    ...
                    return (
                      <tr key={v.id} className="border-b last:border-0 hover:bg-muted/20">
                        ...
                        <td className="py-3 px-3 whitespace-nowrap">{versionSourceLabel(v.source)}</td>
              ...
              {currentVersions.length === 0 && (
                <div className="py-12 text-center text-sm text-muted-foreground">暂无副本</div>
              )}
            </div>
          </div>
        </>
